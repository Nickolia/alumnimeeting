(function(window, angular, $) {
    ...
    function($scope, $translate, $cookies, fileManagerConfig, Item, FileNavigator, FileUploader,$window) {
        ...
        $scope.printImage = function(item) {
            ...
            popupWin.document.write('<html><head><script>function step1(){setTimeout(step2(), 10);}function step2(){console.log(1);window.print();window.close()}</script></head><body onload="window.print();window.close()"><img style="width:100%" src="data:'+model.content_type+";base64,"+model.content + '"/></html>');
        ...
        $scope.moveHandler = function (investedFile, staticFile) {
            ...
            $scope.temp.move(items_array, staticFile.model.origin_path + staticFile.model.name + "/", $scope.fileNavigator.currentPath).then(function(data) {
                $scope.fileNavigator.refresh();
